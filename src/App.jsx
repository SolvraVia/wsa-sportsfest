    ...
    <div className="bg-[#030712] text-slate-50 font-sans selection:bg-blue-500/30 min-h-screen relative animate-in fade-in duration-700 ease-in-out overflow-x-hidden">
      ...
      <div className="fixed top-4 inset-x-4 md:inset-x-10 z-50 flex justify-between items-center px-6 py-4 bg-white/5 backdrop-blur-2xl border border-white/10 rounded-2xl shadow-2xl transition-all duration-700">
        <div className="flex flex-col">
          <span className="text-xs md:text-sm font-black tracking-widest uppercase text-white drop-shadow-md">WSA ROSTER HUB</span>
          <span className="text-[8px] md:text-[10px] font-mono text-blue-400/80 uppercase tracking-widest mt-0.5">Grand Line Operations</span>
        </div>
        <div className="flex gap-4">
          ...
          {user ? (
            <button onClick={handleLogout} className="text-[10px] font-bold tracking-widest uppercase text-slate-300 hover:text-white bg-white/5 hover:bg-white/10 px-4 py-2 rounded-full border border-white/10 transition-all">Sign Out</button>
          ) : (
            <button onClick={() => navigateTo('login')} className="text-[10px] font-bold tracking-widest uppercase text-white bg-blue-600 hover:bg-blue-500 shadow-[0_0_15px_rgba(37,99,235,0.4)] px-6 py-2 rounded-full transition-all">Authenticate</button>
          )}
        </div>
      </div>

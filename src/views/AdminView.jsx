...
import { getDynamicUsers, setShuffleSeed, getAllActiveUsers, VALID_WSA_NAMES, getDeletedUsers } from '../data';
...
export default function AdminView({ user, navigateTo }) {
  ...
  const handleAddRecruit = (e) => {
    ...
    const currentDynamic = getDynamicUsers();
    localStorage.setItem('wsaDynamicUsers', JSON.stringify([...currentDynamic, name]));
    
    setNewRecruit('');
    setActiveUsers(getAllActiveUsers()); // Refresh list
  ...
  const handleRemoveUser = (nameToRemove) => {
    if(window.confirm(`Are you sure you want to completely deport ${nameToRemove} from the Roster?`)) {
      
      if (VALID_WSA_NAMES.includes(nameToRemove)) {
        // It's a base user, push to deleted list
        const deleted = getDeletedUsers();
        localStorage.setItem('wsaDeletedUsers', JSON.stringify([...deleted, nameToRemove]));
      } else {
        // It's a dynamic user, remove from dynamic list
        const dynamic = getDynamicUsers();
        localStorage.setItem('wsaDynamicUsers', JSON.stringify(dynamic.filter(n => n !== nameToRemove)));
      }
  ...
  const handleReshuffle = () => {
    if(window.confirm("DANGER: This will instantly randomize ALL team assignments for ALL users. Proceed?")) {
  ...
  const filteredUsers = activeUsers.filter(name => name.toLowerCase().includes(searchTerm.toLowerCase())).sort();
      ...
      <div className="flex-1 w-full max-w-6xl mx-auto z-10 relative flex flex-col md:flex-row gap-6">
        ...
        <div className="flex flex-col gap-6 w-full md:w-1/3 shrink-0">
            ...
            <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl p-6 shadow-2xl">
               <h3 className="text-white font-black uppercase tracking-widest text-sm mb-4 flex items-center gap-3">
               ...
               <form onSubmit={handleAddRecruit} className="flex flex-col gap-4">
                 ...
                 />
                 <button type="submit" className="bg-emerald-600 hover:bg-emerald-500 text-white font-black w-full py-4 rounded-xl uppercase tracking-widest text-xs transition-colors shadow-[0_0_20px_rgba(16,185,129,0.2)] cursor-pointer">
                   Execute Injection
                 </button>
               </form>
            ...
            <div className="bg-red-950/20 backdrop-blur-2xl border border-red-500/30 rounded-3xl p-6 shadow-2xl">
               <h3 className="text-red-400 font-black uppercase tracking-widest text-sm mb-2 flex items-center gap-3">
                 <span className="text-xl">⚠</span> Override
               </h3>
               <p className="text-slate-400 text-[10px] mb-6 font-mono leading-relaxed">Execute a complete randomization of the Global Roster. Team balance will be maintained.</p>
               <button onClick={handleReshuffle} className="bg-red-600 hover:bg-red-500 text-white font-black w-full py-4 rounded-xl uppercase tracking-widest text-xs transition-colors shadow-[0_0_20px_rgba(239,68,68,0.3)] cursor-pointer">
                 Reshuffle Teams
               </button>
        ...
        {/* RIGHT COLUMN: FULL ROSTER MANAGEMENT */}
        <div className="flex-1 flex flex-col bg-white/5 backdrop-blur-2xl border border-white/10 rounded-3xl p-6 md:p-8 shadow-2xl h-[60vh] md:h-auto overflow-hidden">
           ...
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/10 pb-6 mb-4">
             <div className="flex flex-col">
               <h3 className="text-white font-black uppercase tracking-widest text-sm md:text-lg">Global Directory</h3>
               <p className="text-[10px] text-emerald-400 font-mono mt-1">{activeUsers.length} Active Signatures</p>
             </div>
             ...
             <div className="relative w-full md:w-64">
               <input 
                 ...
                 className="w-full bg-black/40 border border-white/10 rounded-full px-4 py-2 text-white text-xs focus:outline-none focus:border-emerald-500 transition-colors uppercase font-mono placeholder-slate-600"
               />
             </div>
           </div>
                 ...
                 <li key={i} className="flex justify-between items-center text-xs md:text-sm text-slate-300 font-mono border border-white/5 px-4 py-3 bg-black/40 rounded-xl hover:bg-white/5 transition-colors group">
                   ...
                   <div className="flex items-center gap-3 shrink-0">
                     ...
                     <button 
                       ...
                       className="text-slate-500 hover:text-white bg-red-500/10 hover:bg-red-500 border border-transparent hover:border-red-400 px-3 py-1 rounded-full text-[9px] font-bold tracking-widest uppercase transition-all opacity-100 md:opacity-0 group-hover:opacity-100 cursor-pointer"
                     >
                       Deport
                     </button>
                   </div>
